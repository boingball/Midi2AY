...
def basic_loader():
    ...
    lines=[]
    for no,body in ((10,bytes((TOK["LOAD"],))+b' "" '+bytes((TOK["CODE"],))),
...
def block(payload):
    ...
    return body+bytes((sum(payload)&255,))
def tap(name,code):
    basic=basic_loader()
    def header(kind,length,param1,param2):
        return bytes((kind,))+name[:10].encode("ascii").ljust(10,b" ")+struct.pack("<HH",length,param1)+struct.pack("<H",param2)
    result=bytearray()
    result+=block(header(0,len(basic),10,32768)); result+=block(basic)
    result+=block(header(3,len(code),BASE,len(code))); result+=block(code)
    return bytes(result)
